...
function GetTotalRowsNumber() {
    return new Promise( (resolve) => {
        pywebview.api.getTotalRowsNumber('MANGA').then(function(response) {
            resolve(response);
        });
    });
}
...
function GetRowData(index) {
    return new Promise( (resolve) => {
        pywebview.api.getDatabaseRow('MANGA',index).then(function(response) {
            resolve(response);
        });
    });
}
...
function ChangeEpisodesByTextInput(content_id, max_episodes, value, i) {
    ...
    pywebview.api.changeProgressButton(value, 'MANGA', content_id);
}
...
function ChangeEpisodesByButtons(content_id, max_episodes, value, i) {
    ...
    // Invoking the python function to change the progress.
    pywebview.api.changeProgressButton(document.getElementById("actual-progress-"+i).value, 'MANGA', content_id);
}
...
function ChangeScoreByTextInput(content_id, value, i) {
    ...
    // Invoking the python function to change the progress.
    pywebview.api.changeScoreButton(value, 'MANGA', content_id);
}
...
function ChangeScoreByButtons(content_id, value, i) {
    ...
    // Invoking the python function to change the progress.
    pywebview.api.changeScoreButton(document.getElementById("actual-score-"+i).value, 'MANGA', content_id);
}
...
function ShowInfo(content_id) {
    ...
    pywebview.api.dataPreviewButton(parseInt(content_id), 'MANGA').then(function(response) {
        ...
        // Editing some elements in base to the 'MANGA' content type.
...
function RemoveFromMyList(content_id) {
    ...
    pywebview.api.removeButton('MANGA', content_id).then(function() {
...
function GetRecommendation() {
    ...
    pywebview.api.recommendationButton('MANGA').then(function (response) {
